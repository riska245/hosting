...
<header class="fixed top-0 left-0 right-0 z-50 flex justify-center pt-3 sm:pt-4 px-3 sm:px-4 pointer-events-none">
    <div class="app-navbar pointer-events-auto w-full max-w-5xl border rounded-2xl md:rounded-full transition-colors duration-300 overflow-visible">
        ...
        <div class="flex items-center justify-between px-4 sm:px-6 py-3">
            ...
            <div class="flex items-center gap-6 lg:gap-8">
                ...
                <nav class="hidden md:flex items-center gap-1">
                    <a href="/" class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $currentRoute == '/' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'text-black dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-blue-500/5' }}">
                        Beranda
                    </a>
                    <a href="/monitoring" class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $currentRoute == 'monitoring' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'text-black dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-blue-500/5' }}">
                        Monitoring
                    </a>
                    <a href="/pemahaman" class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $currentRoute == 'pemahaman' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'text-black dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-blue-500/5' }}">
                        Edukasi
                    </a>
                    <a href="/about" class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $currentRoute == 'about' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'text-black dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-blue-500/5' }}">
                        Tentang
                    </a>
                    <a href="/kontak" class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $currentRoute == 'kontak' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'text-black dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-blue-500/5' }}">
                        Kontak
                    </a>
                </nav>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <!-- Theme Toggle Button -->
                <button id="theme-toggle" type="button" class="p-2.5 rounded-full text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-white/10 hover:text-blue-700 dark:hover:text-white transition-colors focus:outline-none" title="Toggle Theme">
                    <!-- Sun icon -->
                ...
                <div class="relative hidden sm:block group">
                    ...
                    <input type="text" id="searchInput" autocomplete="off" placeholder="Cari data..." class="app-input w-48 pl-9 pr-4 py-1.5 border rounded-full text-sm placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none transition-all">
                    
                    <!-- Search Results Dropdown -->
                    <div id="searchResults" class="app-popover absolute top-full left-0 right-0 mt-2 border rounded-2xl overflow-hidden hidden max-h-96 flex-col">
                    ...
                    <div class="relative z-[999]" id="profile-dropdown">
                        <button
                            type="button"
                            class="app-primary-button px-5 py-1.5 text-sm font-medium rounded-full focus:outline-none transition-all"
                            id="profile-btn"
                            aria-expanded="false"
                    ...
                    @else
                    <a href="/login" class="app-secondary-button hidden md:block px-5 py-1.5 text-sm font-medium rounded-full hover:bg-blue-50 dark:hover:bg-white/10 transition-all transform hover:-translate-y-0.5 active:translate-y-0 text-center">
                        Masuk
                    </a>
                    @endif

                <!-- Mobile Hamburger Button -->
                <button id="mobile-menu-btn" class="md:hidden p-2.5 rounded-xl text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 focus:outline-none transition-colors active:scale-95">
        ...
        <div id="mobile-menu" class="app-mobile-menu hidden md:hidden border-t backdrop-blur-xl pb-5 transition-colors duration-300 rounded-b-2xl">
            
            <!-- Mobile Search -->
            <div class="px-4 pt-4 pb-2 sm:hidden">
            ...
            <nav class="flex flex-col px-3 sm:px-4 pt-2 space-y-1">
                ...
                <div class="pt-3 px-1">
                    ...
                    @else
                    <a href="/login" class="flex items-center justify-center gap-2 w-full px-5 py-3.5 text-base font-semibold text-white dark:text-white bg-slate-900 dark:bg-blue-600 rounded-xl hover:bg-slate-800 dark:hover:bg-blue-500 active:scale-[0.98] transition-all text-center">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>
                        Masuk Akun
                    </a>
                    @endif
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchSeeMore = document.getElementById('searchSeeMore');
    const searchHiddenCount = document.getElementById('searchHiddenCount');
    ...
    function renderResults(matches, showAll = false) {
        ...
        if (matches.length > 5 && !showAll) {
            searchSeeMore.classList.remove('hidden');
            searchHiddenCount.textContent = matches.length - 5;
        } else {
            searchSeeMore.classList.add('hidden');
        }
    ...
    searchSeeMore.addEventListener('click', function(e) {
        e.preventDefault();
        isShowingAll = true;
    ...
    // Focus search input brings back results if there's query
